...
describe('createUrlBuilder', () => {
    it('builds platform URLs by application surface', () => {
        ...
        expect(builder.platformPublic('/services')).toBe('https://webuildyouthrive.com/services');
        expect(builder.platformAuth('/login')).toBe('https://accounts.webuildyouthrive.com/login');
        expect(builder.platformAdmin('/tenants')).toBe('https://admin.webuildyouthrive.com/tenants');
    });
    ...
    it('builds tenant URLs with query and hash fragments', () => {
        ...
        expect(
            builder.tenantPublic('/request-service', {
                query: { location: 'houston', emergency: true, department: ['hvac', 'electrical'] },
                hash: 'top',
            }),
        ).toBe(
